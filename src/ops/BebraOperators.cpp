...
std::vector<int64_t> OpVoid::countOutputShape(const Core::BebraGraph& graph) {    return {};
}
std::vector<int64_t> OpConv::countOutputShape(const Core::BebraGraph& graph) {    auto inputShape = graph.getTensor(input).getShape(); // [N, C, H, W]
...
std::vector<int64_t> OpGemm::countOutputShape(const Core::BebraGraph& graph) {    auto shapeA = graph.getTensor(input_a).getShape();
    auto shapeB = graph.getTensor(input_b).getShape();
    // [M, K] x [K, N] -> [M, N]
    return calculateMatMulShape(shapeA, shapeB);
}
std::vector<int64_t> OpAdd::countOutputShape(const Core::BebraGraph& graph) {    auto shapeA = graph.getTensor(input_1).getShape();
    auto shapeB = graph.getTensor(input_2).getShape();
    return calculateBroadcastShape(shapeA, shapeB);
}
std::vector<int64_t> OpRelu::countOutputShape(const Core::BebraGraph& graph) {return graph.getTensor(input).getShape();}
std::vector<int64_t> OpMul::countOutputShape(const Core::BebraGraph& graph) {    auto shapeA = graph.getTensor(input_1).getShape();
    auto shapeB = graph.getTensor(input_2).getShape();
    return calculateBroadcastShape(shapeA, shapeB);
}
std::vector<int64_t> OpMatMul::countOutputShape(const Core::BebraGraph& graph) {    auto shapeA = graph.getTensor(input_a).getShape();
    auto shapeB = graph.getTensor(input_b).getShape();
    // [M, K] x [K, N] -> [M, N]
    return calculateMatMulShape(shapeA, shapeB);
}
std::vector<int64_t> OpMaxPool::countOutputShape(const Core::BebraGraph& graph) {  const auto& inputShape = graph.getTensor(input).getShape();
...
std::vector<int64_t> OpReduceMean::countOutputShape(const Core::BebraGraph& graph) {    auto inputShape = graph.getTensor(input).getShape();
    return calculateReduceShape(inputShape, axes, keepdims);
}
std::vector<int64_t> OpReshape::countOutputShape(const Core::BebraGraph& graph) {return graph.getTensor(shape).getShape();}
std::vector<int64_t> OpSigmoid::countOutputShape(const Core::BebraGraph& graph) {return graph.getTensor(input).getShape();}
std::vector<int64_t> OpFlatten::countOutputShape(const Core::BebraGraph& graph) {return calculateFlattenShape(graph.getTensor(input).getShape(), axis);}
}
