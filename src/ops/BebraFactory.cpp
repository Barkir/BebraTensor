...
OpVariant CreateOpConv(const onnx::NodeProto& node,
    ...
    op.input = inputs[0];
    op.weight = inputs[1];
    if (inputs.size() > 2 && !inputs[2].empty())
        op.bias = inputs[2];
...

    {
        ...
        if (it == attrs.end())
            throw Core::BebraErr("Conv: missing required attr 'kernel_shape'");
        op.kernel_shape = it->second.getValRef<std::vector<int64_t>>();
    }
    {
        auto it = attrs.find("group");
        op.group = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(1);
    }
    {
        auto it = attrs.find("dilations");
        op.dilations =
            (it != attrs.end()) ? it->second.getValRef<std::vector<int64_t>>() : std::vector<int64_t>({1, 1});
    }
    {
        auto it = attrs.find("pads");
        op.pads = (it != attrs.end()) ? it->second.getValRef<std::vector<int64_t>>() : std::vector<int64_t>({0, 0});
    }
    {
        auto it = attrs.find("strides");
        op.strides = (it != attrs.end()) ? it->second.getValRef<std::vector<int64_t>>() : std::vector<int64_t>({1, 1});
    }
    return op;
}
OpVariant CreateOpGemm(const onnx::NodeProto& node,
...

    {
        ...
        op.alpha = (it != attrs.end()) ? it->second.getValRef<float>() : float(1.0);
    }
    {
        auto it = attrs.find("beta");
        op.beta = (it != attrs.end()) ? it->second.getValRef<float>() : float(1.0);
    }
    {
        auto it = attrs.find("transA");
        op.transA = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(0);
    }
    {
        auto it = attrs.find("transB");
        op.transB = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(0);
    }
    return op;
}
...
OpVariant CreateOpMaxPool(const onnx::NodeProto& node,
    ...
    op.output = outputs[0];
    if (outputs.size() > 1)
        op.indices = outputs[1];

    {
        auto it = attrs.find("kernel_shape");
        if (it == attrs.end())
            throw Core::BebraErr("MaxPool: missing required attr 'kernel_shape'");
        op.kernel_shape = it->second.getValRef<std::vector<int64_t>>();
    }
    {
        auto it = attrs.find("auto_pad");
        op.auto_pad = (it != attrs.end()) ? it->second.getValRef<std::string>() : std::string("NOTSET");
    }
    {
        auto it = attrs.find("ceil_mode");
        op.ceil_mode = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(0);
    }
    {
        auto it = attrs.find("dilations");
        op.dilations =
    ...
    }
    {
        ...
        op.storage_order = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(0);
    }
    {
        auto it = attrs.find("strides");
        op.strides = (it != attrs.end()) ? it->second.getValRef<std::vector<int64_t>>() : std::vector<int64_t>({1, 1});
...
OpVariant CreateOpReduceMean(const onnx::NodeProto& node,
...

    {
        ...
        if (it == attrs.end())
            throw Core::BebraErr("ReduceMean: missing required attr 'axes'");
        op.axes = it->second.getValRef<std::vector<int64_t>>();
    }
    {
        auto it = attrs.find("keepdims");
        op.keepdims = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(1);
    }
    return op;
}
...
OpVariant CreateOpFlatten(const onnx::NodeProto& node,
...

    {
        ...
        op.axis = (it != attrs.end()) ? it->second.getValRef<int64_t>() : int64_t(1);
    }
    return op;
}
} // namespace Bebra::Ops
